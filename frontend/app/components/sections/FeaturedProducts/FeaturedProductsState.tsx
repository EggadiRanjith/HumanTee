...
export function FeaturedProductsState({ type, message }: ErrorStateProps) {
    const isError = type === 'error';

    // Select the appropriate animation
    const animationData = isError ? errorAnimation : emptyAnimation;

    const title = isError ? 'Something Went Wrong' : 'No Featured Products';
    const description = message || (isError
    ...
    return (
        ...
        "
            >
                {isError ? 'TRY SHOP PAGE' : 'BROWSE SHOP'}
            </Link>
        </div>
    );
}
