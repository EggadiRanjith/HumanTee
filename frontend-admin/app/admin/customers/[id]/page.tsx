        ...
        <div className="space-y-6 max-w-7xl mx-auto">
            ...
            <div className="flex items-center gap-4">
                <Link href="/admin/customers">
                    <button className="p-2 hover:bg-gray-100 rounded-lg transition-colors border border-gray-200 bg-white">
                        <FiArrowLeft className="w-5 h-5" />
                    </button>
                </Link>
                <div>
                    <h1 className="text-2xl font-bold text-black">{customer.profile?.full_name || 'Customer Detail'}</h1>
                    <p className="text-gray-500 text-sm">Reviewing customer history and profiles</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
                        <div className="h-24 bg-gradient-to-r from-gray-900 to-black" />
                        <div className="px-6 pb-6">
                            <div className="relative -mt-10 mb-4">
                                <div className="w-20 h-20 rounded-2xl bg-white p-1 border border-gray-100 shadow-md">
                                    <div className="w-full h-full rounded-xl bg-gray-50 flex items-center justify-center text-2xl font-bold text-black border border-gray-100">
                                        {customer.profile?.full_name?.charAt(0) || 'U'}
                                    </div>
                                </div>
                            </div>
                            <h3 className="text-lg font-bold text-black">{customer.profile?.full_name || 'None'}</h3>
                            <p className="text-xs text-gray-500 mb-6 lowercase tracking-wider">{customer.role} • ID: {customer.id.split('-')[0]}...</p>
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1 text-right">
                            ...
                            <p className="text-xl font-bold text-black">₹{totalSpend.toFixed(2)}</p>
                        </div>
                    </div>
                </div>

                {/* Activity Tabs (Orders & Tickets) */}
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                            ...
                            <span className="text-[10px] font-bold text-gray-400">{customer.orders?.length || 0} Total</span>
                        </div>
                        <div className="divide-y divide-gray-100">
                            {customer.orders?.length > 0 ? (
                                customer.orders.map((order: any) => (
                                    <div key={order.id} className="p-6 hover:bg-gray-50 transition-colors flex items-center justify-between group">
                                        ...
                                        <div className="flex items-center gap-6">
                                            <p className="text-sm font-bold text-black">₹{Number(order.totalAmount).toFixed(2)}</p>
                                            <Link href={`/admin/orders/${order.id}`}>
                                                <button className="p-2 rounded-lg border border-gray-100 group-hover:bg-white group-hover:border-black transition-all">
                                ...
                                <div className="p-12 text-center text-gray-400 text-sm">No orders found for this customer.</div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                            <h3 className="text-xs font-bold text-black uppercase tracking-widest flex items-center gap-2">
                                <FiMessageSquare className="w-4 h-4" /> Support Tickets
                            </h3>
                            <span className="text-[10px] font-bold text-gray-400">{customer.tickets?.length || 0} Total</span>
                        </div>
                        <div className="divide-y divide-gray-100">
                            {customer.tickets?.length > 0 ? (
                                    ...
                                    <div key={ticket.id} className="p-6 hover:bg-gray-50 transition-colors flex items-center justify-between group">
                                        <div>
                                            <div className="flex items-center gap-2">
                                                <p className="text-sm font-bold text-black">{ticket.subject}</p>
                                ...
                                <div className="p-12 text-center text-gray-400 text-sm">No support tickets found.</div>
                            )}
                        </div>
                    </div>
                </div>
